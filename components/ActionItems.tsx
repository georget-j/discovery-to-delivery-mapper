...
export function ActionItems() {
  ...
  const allItems = useMemo(
    () => sessions.flatMap((s) =>
      s.actionItems.map((item) => ({ item, sessionId: s.id, sessionTitle: s.title || "Untitled session" }))
    ),
    [sessions]
  );
  ...
  const sorted = useMemo(() => {
    const URGENCY_RANK: Record<ActionItemUrgency, number> = { high: 0, medium: 1, low: 2 };
    return [...filtered].sort((a, b) => {
      const aOverdue = a.item.dueDate && /^\d{4}-\d{2}-\d{2}$/.test(a.item.dueDate) && a.item.dueDate < today;
      const bOverdue = b.item.dueDate && /^\d{4}-\d{2}-\d{2}$/.test(b.item.dueDate) && b.item.dueDate < today;
  ...
  const overdueCount = allItems.filter(({ item }) =>
    item.status === "open" && item.dueDate && /^\d{4}-\d{2}-\d{2}$/.test(item.dueDate) && item.dueDate < today
  ).length;
      ...
      meetingSessions: sessions.map((s) =>
        s.id !== sessionId ? s : {
          ...
          actionItems: s.actionItems.map((a) =>
            a.id !== item.id ? a : { ...a, status: a.status === "done" ? "open" : "done" as const }
          ),
        }
      ),
    });
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
          ...
          <span className="ml-2 text-xs font-normal normal-case text-muted-foreground/80">
            {openCount} open
            {overdueCount > 0 && <span className="text-red-700 font-medium"> · {overdueCount} overdue</span>}
          </span>
        </h2>
      ...
      {sorted.length === 0 ? (
        <div className="rounded-lg border bg-muted/20 px-4 py-3 text-xs text-muted-foreground italic">
          {filter === "open" ? "No open action items." : filter === "overdue" ? "Nothing overdue. ✓" : "No action items in any session."}
        </div>
      ) : (
        <div className="rounded-lg border bg-background divide-y">
          {sorted.map(({ item, sessionId, sessionTitle }) => {
            const isDone = item.status === "done";
            const isOverdue = item.dueDate && /^\d{4}-\d{2}-\d{2}$/.test(item.dueDate) && item.dueDate < today && !isDone;
            return (
              <div key={item.id} className="flex items-start gap-3 px-3 py-2">
                ...
                <div className="flex-1 min-w-0 space-y-0.5">
                  ...
                  <div className="flex items-center gap-2 text-[11px] text-muted-foreground flex-wrap">
                    ...
                    <span className="text-muted-foreground/60">from: {sessionTitle}</span>
                  </div>
                </div>
                <span className={cn("text-[10px] px-1.5 py-0.5 rounded border shrink-0 mt-0.5 capitalize", URGENCY_STYLE[item.urgency])}>
                  {item.urgency}
                </span>
              </div>
            );
