...
const TYPE_STYLES: Record<WorkflowNodeType, { container: string; label: string }> = {
  ...
  decision:     { container: "border-amber-300 bg-amber-50/80 rotate-0", label: "text-amber-900" },
...
export type WorkflowNodeRenderData = WorkflowNodeData & { selected?: boolean };

function WorkflowNodeImpl({ data, selected }: NodeProps) {
  const node = data as unknown as WorkflowNodeRenderData;
  const style = TYPE_STYLES[node.type];

  if (node.type === "decision") {
    // Diamond shape — rotated square with un-rotated text
    return (
