...
export function CoverageMatrix({ project, onPickArtifact }: Props) {
  ...
  const stale = !!project.outputs?.derivedFromHash && project.outputs.derivedFromHash !== hash;

  // Precompute the whole grid
  const grid = useMemo(() => {
    const result: Record<CoverageInputCategory, Record<ArtifactKey, { strength: CoverageStrength; count: number }>> = {} as never;
  ...
  const captured = (cat: CoverageInputCategory): boolean => {
    switch (cat) {
      case "customer":     return !!project.customer.companyName;
      case "discovery":    return !!project.discovery.currentProcess || !!project.discovery.successDefinition;
      case "regulatory":   return (project.customer.regulatoryContext?.length ?? 0) > 0;
      case "workflows":    return project.workflows.length > 0;
      case "systems":      return project.systems.length > 0;
      case "data_sources": return project.dataSources.length > 0;
      case "stakeholders": return project.stakeholders.length > 0;
      case "risks":        return project.risks.length > 0;
      case "pilot":        return !!project.pilotPlan?.objective;
    }
  };

  const visibleCategories = emptyOnly ? CATEGORIES.filter((c) => !captured(c.key)) : CATEGORIES;
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
            Which inputs feed which artifact. Solid = all entries used. Hollow = some used. Empty = not used.
      ...
      {stale && (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
          Inputs have changed since the last generation. Regenerate from the top of the Outputs page to refresh.
        </div>
      )}
                ...
                <tr key={cat.key} className="border-b last:border-b-0">
                  <td className="px-3 py-2 align-middle">
                    <div className="flex items-center gap-2">
                      <span className={cn("w-2 h-2 rounded-full shrink-0", isCaptured ? cat.dot : "bg-muted-foreground/30")} />
                      <span className={cn("font-medium", !isCaptured && "text-muted-foreground italic")}>
                        {cat.label}
                      </span>
                      {!isCaptured && <span className="text-[10px] text-muted-foreground/70">(empty)</span>}
                    </div>
                  </td>
                  {ARTIFACT_KEYS_ORDERED.map((art) => {
                    ...
                    return (
                      <td key={art} className="text-center px-1 py-2">
                        <DotCell strength={cell.strength} count={cell.count} categoryDot={cat.dot} captured={isCaptured} />
                      </td>
                    );
                  })}
                </tr>
...
function DotCell({
  strength, count, categoryDot, captured,
}: {
  ...
  if (strength === "none") return <span className="text-muted-foreground/30">·</span>;
  if (!captured) {
    return <span className="text-muted-foreground/30" title={`Would use ${count}, but category is empty`}>○</span>;
  }
  if (strength === "solid") {
    return (
      <span
  ...
  return (
    <span
      className={cn("inline-block w-2.5 h-2.5 rounded-full border-2 bg-background", categoryDot.replace("bg-", "border-"))}
