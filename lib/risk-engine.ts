...
export function generateRisks(project: OnboardingProject): DeploymentRisk[] {
      ...
      results.push(
        risk(
          ...
          "Scope a custom connector or file-based pipeline. Add 4–6 weeks buffer to integration timeline.",
          {
            owner: "engineering",
            escalationTrigger:
              "Integration timeline exceeds 6 weeks or vendor refuses to provide data access",
            sourceRefs: [
              { type: "system", refId: system.id, label: system.name },
            ],
            rationale: `${system.name} is flagged as having no API available — a custom integration is required.`,
          },
        ),
      );
    }
      ...
      results.push(
        risk(
          ...
          "Conduct a dedicated technical scoping session with the customer's IT team before committing to a timeline.",
          {
            ...
            rationale: `${system.name} is rated high integration complexity in the Systems tab.`,
          },
        ),
      );
    }
      ...
      results.push(
        risk(
          ...
          "Allocate dedicated data quality sprint before pilot. Define minimum quality thresholds as a launch criterion.",
          {
            owner: "customer",
            escalationTrigger:
              "Data quality below agreed threshold at T-4 weeks before pilot start",
            sourceRefs: [
              { type: "data_source", refId: source.id, label: source.name },
            ],
            rationale: `${source.name} has data quality marked "poor" in the Systems tab.`,
          },
        ),
      );
    }
      ...
      results.push(
        risk(
          ...
          "Escalate to customer executive sponsor immediately. Define a hard unblock deadline as a launch criterion.",
          {
            owner: "customer",
            escalationTrigger:
              "Access not unblocked within 2 weeks of project start",
            sourceRefs: [
              { type: "data_source", refId: source.id, label: source.name },
            ],
            rationale: `${source.name} has access status marked "blocked" — the pilot cannot run without it.`,
          },
        ),
      );
    }
  ...
  const regulatedSystems = project.systems.filter(
    (s) => s.dataSensitivity === "regulated",
  );
  const piiSources = project.dataSources.filter((d) => d.pii === true);
  const hasRegulatedData = regulatedSystems.length > 0 || piiSources.length > 0;

  if (hasRegulatedData) {
    const sources = [
      ...regulatedSystems.map((s) => s.name),
      ...piiSources.map((d) => d.name),
    ];
    results.push(
      risk(
        ...
        "Complete security review and data processing agreement before any data is transferred. Implement encryption at rest and in transit.",
        {
          owner: "security",
          escalationTrigger:
            "DPA not signed 4 weeks before pilot start or security review fails",
          rationale: `Regulated/PII data was flagged on ${sources.slice(0, 2).join(", ")}${sources.length > 2 ? ` and ${sources.length - 2} more` : ""}.`,
        },
      ),
    );
  }
    ...
    results.push(
      risk(
        ...
        "Require a named technical owner as a pre-condition of project kick-off. Escalate to executive sponsor if not provided.",
        {
          owner: "customer",
          escalationTrigger:
            "Technical owner not named within 1 week of project start",
        },
      ),
    );
  }
  ...
  // ── Timeline risks ─────────────────────────────────────────────────────────
  if (
    project.customer.urgency === "critical" &&
    ...
    results.push(
      risk(
        ...
        "Hold a mandatory discovery completion session before any delivery work begins. Communicate the risk of starting without full discovery.",
        {
          owner: "shared",
          escalationTrigger:
            "Discovery not completed within 2 weeks of project start",
        },
      ),
    );
  }
  ...
  if (
    project.customer.urgency === "critical" ||
    project.customer.urgency === "high"
  ) {
    const hasBlockedData = project.dataSources.some(
      (d) => d.accessStatus === "blocked",
    );
      ...
      results.push(
        risk(
          ...
          "Surface blockers to executive sponsor immediately. Negotiate a realistic go-live date that accounts for resolution time.",
          {
            owner: "shared",
            escalationTrigger:
              "Blockers not resolved by T-3 weeks before agreed pilot start",
          },
        ),
      );
    }
    ...
    results.push(
      risk(
        ...
        "Engage customer legal and procurement teams in week 1. Provide compliance documentation pack early to avoid last-minute delays.",
        {
          owner: "customer",
          escalationTrigger:
            "Legal/procurement review not started within 2 weeks of project kick-off",
        },
      ),
    );
  }
  ...
  if (project.customer.industry === "fintech") {
    results.push(
      risk(
        ...
        {
          owner: "startup",
          escalationTrigger: "Model fails explainability test in pilot review",
        },
      ),
    );
    results.push(
  ...
  if (project.customer.industry === "legaltech") {
    results.push(
      risk(
        ...
        {
          owner: "startup",
          escalationTrigger: "Hallucination rate exceeds 2% in pilot testing",
        },
      ),
    );
    results.push(
      risk(
        ...
        "Require legal opinion on privilege implications before any client data is processed. Implement strict data segregation and consent controls.",
        {
          owner: "shared",
          escalationTrigger:
            "Any privileged data is inadvertently included in training or inference without consent",
        },
      ),
    );
  }
